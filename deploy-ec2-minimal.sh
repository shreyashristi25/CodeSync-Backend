...
if [ ! -f .env.ec2-minimal ]; then
  echo "Missing .env.ec2-minimal. Copy env/ec2-minimal/.env.ec2-minimal.example to .env.ec2-minimal first." >&2
  exit 1
fi

missing_env=0
for example in env/ec2-minimal/*.env.example; do
